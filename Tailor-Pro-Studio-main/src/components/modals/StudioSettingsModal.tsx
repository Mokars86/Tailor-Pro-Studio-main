...
export const StudioSettingsModal: React.FC<StudioSettingsModalProps> = ({
  ...
  const [unbindNotice, setUnbindNotice] = useState<string | null>(null);
  const [apprenticeToUnbind, setApprenticeToUnbind] = useState<Apprentice | null>(null);
  const [unbindFilterQuery, setUnbindFilterQuery] = useState<string>('');

  const filteredApprenticesForUnbind = (apprentices || []).filter((app) => {
    if (!app) return false;
    if (!unbindFilterQuery || !unbindFilterQuery.trim()) return true;
    const q = unbindFilterQuery.toLowerCase();
    const appName = (app.name || '').toLowerCase();
    const appRole = (app.role || '').toLowerCase();
    const appMentor = (app.mentor || '').toLowerCase();
    return appName.includes(q) || appRole.includes(q) || appMentor.includes(q);
  });
        ...
        <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-5 text-xs">
          ...
          <div className="p-3 sm:p-4 rounded-2xl bg-rose-500/10 dark:bg-rose-950/30 border border-rose-300/60 dark:border-rose-800/60 space-y-3 font-['Outfit']">
              ...
              <div className="flex items-center gap-1.5 shrink-0 flex-wrap">
                ...
                {apprentices && apprentices.length > 1 && (
                  <button
                    type="button"
                    onClick={() => {
                      if (window.confirm(`Are you sure you want to unbind and unlink ALL ${apprentices.length} apprentices from your Master Studio?`)) {
                        [...apprentices].forEach((app) => {
                          if (onUnlinkApprentice) onUnlinkApprentice(app.id);
                        });
                        setUnbindNotice(`All ${apprentices.length} apprentices have been unlinked from your studio!`);
            ...
            <p className="text-[11px] font-medium text-slate-700 dark:text-slate-300 leading-relaxed">
              Master control feature to <strong>unlink or unbind an apprentice</strong> from your studio. Unbinding will disconnect their live sync pairing with your workshop code and remove their profile from your active registry.
            </p>

            {/* Search Filter for Apprentices if > 2 */}
            {apprentices && apprentices.length > 2 && (
            ...
            {(!apprentices || apprentices.length === 0) ? (
              <div className="p-3 rounded-xl bg-white/80 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 text-xs font-semibold text-slate-500 dark:text-slate-400 text-center">
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (onUnlinkApprentice) {
                        onUnlinkApprentice(apprenticeToUnbind.id);
                        setUnbindNotice(`${apprenticeToUnbind.name} has been unlinked and disconnected from your studio!`);
                        setApprenticeToUnbind(null);
                        setTimeout(() => setUnbindNotice(null), 3500);
                      }
                    }}
            ...
            {unbindNotice && (
              <div className="p-2.5 rounded-xl bg-rose-100 dark:bg-rose-900/60 text-rose-800 dark:text-rose-200 text-xs font-bold text-center border border-rose-300 dark:border-rose-700 animate-fade-in">
                ✓ {unbindNotice}
              </div>
            )}
          </div>
